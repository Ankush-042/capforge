...
function HeroDiagram() {
  return (
    <div className="hero-diagram" aria-label="CapForge team formation visualization">
      ...
      <div className="signal-readout"><span>LIVE SIGNAL</span><strong>+18.4%</strong><svg viewBox="0 0 120 34" aria-hidden="true"><path d="M2 28 L18 23 L30 25 L42 14 L56 18 L69 8 L82 13 L96 4 L118 7" /></svg><small>team fit trajectory</small></div>
...
export default function CapForgeLanding() {
  ...
  return <main id="top" onPointerMove={handlePointerMove}>
    <header className="site-header"><Logo /><div className="nav-shell"><span className="nav-status"><span className="status-dot" />AI ecosystem</span><nav className="desktop-nav" aria-label="Primary navigation">{navItems.map(([label, href], i) => <a href={href} className={i === 0 ? 'nav-active' : ''} key={label}><span>{label}</span>{i === 0 && <i />}</a>)}</nav></div><div className="header-actions"><a className="sign-in" href="/sign-in">Sign In</a><a className="button button-dark small" href="/sign-up">Get Started <ArrowUpRight size={15} /></a></div><button className="menu-toggle" onClick={() => setMenuOpen(!menuOpen)} aria-label={menuOpen ? 'Close menu' : 'Open menu'}>{menuOpen ? <X /> : <Menu />}</button></header>
    {menuOpen && <nav className="mobile-nav">{navItems.map(([label, href]) => <a href={href} key={label} onClick={() => setMenuOpen(false)}>{label}<ArrowUpRight size={15} /></a>)}<a className="button button-dark" href="/sign-up">Get Started</a></nav>}

    <section className="hero section-wrap"><div className="hero-copy"><SectionLabel>The intelligent startup ecosystem</SectionLabel><h1>Build the team your startup was <em>missing.</em></h1><p>CapForge uses AI to understand what your startup needs, identify what&apos;s missing, and connect you with the people who can help build it.</p><div className="hero-actions"><a href="/sign-up" className="button button-dark">Build Your Startup <ArrowUpRight size={16} /></a><a href="#discover" className="text-link">Explore Startups <ArrowDownRight size={16} /></a></div><span className="scroll-cue">Scroll to explore <ArrowDownRight size={14} /></span></div><HeroDiagram /></section>

    <section className="ecosystem section-wrap" id="discover"><div className="ecosystem-dark-card"><img src="/capforge-dark-ecosystem.png" alt="CapForge intelligence network connecting startup, talent, and capital signals" /><div className="ecosystem-overlay"><div><SectionLabel>CapForge / ecosystem signal</SectionLabel><h2>One intelligence layer.<br /><em>Every connection made useful.</em></h2><p>From founder intent to team fit, opportunity, and capital — CapForge sees the system around the startup.</p></div><a href="/sign-up" className="button button-light">Explore the network <ArrowUpRight size={15} /></a></div><div className="ecosystem-metrics"><span><b>01</b> Founder intent</span><span><b>02</b> Capability gaps</span><span><b>03</b> Aligned capital</span><span className="ecosystem-live"><i /> Live signal map</span></div></div></section>

    <section className="intelligence-strip section-wrap" aria-label="CapForge intelligence overview"><div className="intelligence-art"><img src="/capforge-intelligence.png" alt="Abstract CapForge startup intelligence profile with connected team and opportunity signals" /><div className="art-stamp">CF / SIGNAL MAP <span>LIVE</span></div><div className="art-cursor">AI</div><div className="art-caption"><span>STARTUP INTELLIGENCE PROFILE</span><strong>AgriVision</strong><small>Vision · Team · Skills · Traction</small></div><div className="art-axis"><span>TEAM</span><span>SKILL GAP</span><span>CAPITAL</span></div></div><div className="signal-copy"><SectionLabel>One intelligence layer</SectionLabel><h2>See the gap. Find the fit. <em>Move forward.</em></h2><p>CapForge turns a startup into a living intelligence profile — then connects every missing capability to the people, opportunities, and capital that can change its trajectory.</p><div className="signal-points"><div><strong>01</strong><span>Team builder</span><small>Roles your startup actually needs</small></div><div><strong>02</strong><span>Opportunity match</span><small>Projects aligned to your strengths</small></div><div><strong>03</strong><span>Capital fit</span><small>Investors who understand your thesis</small></div></div></div></section>
    ...
    <section className="connection section-wrap" id="connect"><SectionLabel>The connection</SectionLabel><h2>Discovery is only<br /><em>the beginning.</em></h2><div className="sequence" role="tablist" aria-label="CapForge journey">{['Understand', 'Match', 'Connect', 'Form Team', 'Build'].map((x, i) => <button type="button" role="tab" aria-selected={activeStage === i} className={activeStage === i ? 'sequence-active' : ''} onClick={() => setActiveStage(i)} key={x}><span>0{i + 1}</span><strong>{x}</strong>{i < 4 && <ArrowUpRight size={15} />}</button>)}</div><div className="journey-stage"><div className="journey-copy"><span className="tiny-label">STAGE 0{activeStage + 1} / CAPFORGE ENGINE</span><h3>{['A startup comes into focus.', 'The missing capabilities become clear.', 'The right people move closer.', 'A compatible team takes shape.', 'The work starts moving forward.'][activeStage]}</h3><p>{['CapForge reads the vision, market, team, and ambition behind the idea.', 'AI maps what exists against what is needed next — without guesswork.', 'Skills, experience, working style, and intent become one useful signal.', 'The strongest composition rises above an endless directory of profiles.', 'The new team gets a shared workspace to turn alignment into progress.'][activeStage]}</p><a href="/sign-up" className="text-link">Continue the journey <ArrowUpRight size={15} /></a></div><div className={`journey-visual journey-${activeStage}`}><div className="journey-grid" /><div className="journey-core"><span>CF</span><strong>{['IDEA', 'GAP', 'FIT', 'TEAM', 'BUILD'][activeStage]}</strong></div><span className="journey-node jn-one">VISION</span><span className="journey-node jn-two">SKILL</span><span className="journey-node jn-three">PEOPLE</span><div className="journey-path" /></div></div><Workspace /></section>
    ...
    <section className="closing section-wrap" id="start"><div><SectionLabel>Start here</SectionLabel><h2>Build something worth building.<br /><em>With the right people.</em></h2><p>CapForge turns startup discovery into startup formation.</p></div><div className="closing-actions"><a className="button button-dark" href="/sign-up">Get Started <ArrowUpRight size={16} /></a><a className="text-link" href="#discover">Explore the ecosystem <ArrowDownRight size={16} /></a></div></section>
