...
export const getAdminUsers = (search) => apiFetch(`/admin/users${search ? `?search=${encodeURIComponent(search)}` : ''}`);
export const getAdminStartups = (search) => apiFetch(`/admin/startups${search ? `?search=${encodeURIComponent(search)}` : ''}`);
