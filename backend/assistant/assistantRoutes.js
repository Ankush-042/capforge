...
const { askAboutVenture, askAsContributor } = require('./assistantService');
...
// The same assistant from the other side of the table. A contributor could
// ask nothing about the ventures courting them.
router.post('/ask-about-my-matches', requireAuth, aiEndpointLimit, async (req, res) => {
  const result = await askAsContributor(req.user.userId, req.body?.question);
